...
class Tm_Cmsblock extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('TM - CMS Block', array(), 'Modules.CmsBlock');
    ...
    public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
                ...
                `text` text NOT NULL,
                PRIMARY KEY (`id_tmcmsblockinfo`, `id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;'
        );
    ...
    public function uninstallDB($drop_table = true)
    {
        $ret = true;
        if ($drop_table) {
            $ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'tmcmsblockinfo`') && Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'tmcmsblockinfo_lang`');
        }
   ...
   public function processSaveCmsBlock()
    {
		...
		if (Shop::isFeatureActive() && !$tmcmsblockinfo->id_shop) {
            $saved = true;
            $shop_ids = Shop::getShops();
            foreach ($shop_ids as $id_shop) {
                $tmcmsblockinfo->id_shop = $id_shop;
                $saved &= $tmcmsblockinfo->add();
            }
        } else {
			$tmcmsblockinfo->id_shop = Shop::getContextShopID();
    ...
    public function getFormValues()
    {
        ...
        foreach (Language::getLanguages(false) as $lang) {
            $tmcmsblockinfo = new CmsBlock((int)$id_tmcmsblockinfo);
            $fields_value['text'][(int)$lang['id_lang']] = $tmcmsblockinfo->text[(int)$lang['id_lang']];
        }

        $fields_value['id_tmcmsblockinfo'] = $id_tmcmsblockinfo;
    ...
    {
        $sql = 'SELECT r.`id_tmcmsblockinfo`, r.`id_shop`, rl.`text`
            FROM `'._DB_PREFIX_.'tmcmsblockinfo` r
            LEFT JOIN `'._DB_PREFIX_.'tmcmsblockinfo_lang` rl ON (r.`id_tmcmsblockinfo` = rl.`id_tmcmsblockinfo`)
