...
class cdc_googletagmanager extends Module
{
    ...
    public function hookDisplayAfterTitle($params)
    {
        ...
        if($this->gtm_enable && !empty($this->gtm_id)) {
            ...
            $this->context->smarty->assign(array('always_display_variant_id' => $this->always_display_variant_id));
                ...
                switch ($controller) {
                    ...
                    case self::PAGE_PRODUCT:
                        ...
                        if(Validate::isLoadedObject($product)) {
                            $this->dataLayer->google_tag_params->ecomm_prodid = $this->getProductIdentifier($product);
    ...
    protected function getProductIdentifier($product) {
        ...
        if($this->always_display_variant_id && ($id_type == 'id' || $id_type == 'id_product')) {
            ...
            $product_identifier .= '-'.$product_attribute;
        }
     ...
     * @return String
     */
    protected function getProductName($product, $id_lang) {
        switch ($this->product_name_field) {
            case 'link_rewrite':
                if(!empty($product->link_rewrite)) {
                    if(is_array($product->link_rewrite)) {
                        $product_name = !empty($product->link_rewrite[$id_lang]) ? $product->link_rewrite[$id_lang] : $product->link_rewrite[0];
                    } else {
                        $product_name = $product->link_rewrite;
                    }
                }
                break;
            ...
            default:
                $product_name = $this->cleanString(Product::getProductName($product->id, 0, $id_lang));
                break;
        }
     ...
     */
    protected function getCategoryName($id_category, $id_lang = null) {
        $categoryName = '';
        ...
        switch ($this->category_name_field) {
            case 'link_rewrite':
                if(!empty($category->link_rewrite)) {
                    if(is_array($category->link_rewrite)) {
                        $categoryName = !empty($category->link_rewrite[$id_lang]) ? $category->link_rewrite[$id_lang] : $category->link_rewrite[0];
                    } else {
    ...
    protected function getDataLayerProduct($product, $category_name = null) {
        ...
        $dataLayerProduct->id = (string) $product->id;
        ...
        // product name
        $dataLayerProduct->name = $this->getProductName($product, $this->context->language->id);
