...
class WebPayValidateModuleFrontController extends ModuleFrontController {
	...
	private function processPayment($data) {
        ...
        if (isset($result->buyOrder) && isset($result->detailOutput) && $result->detailOutput->responseCode == 0) {
            ...
            $payment[0]->card_number = $result->detailOutput->paymentTypeCode .'-'. $result->cardDetail->cardNumber;
            $payment[0]->card_brand = $result->detailOutput->authorizationCode;
            $payment[0]->card_expiration = $result->detailOutput->paymentTypeCode;
            ...
            if (isset($payment[0])) {
                $payment[0]->transaction_id = $cart->id;
                $payment[0]->card_number =$paymentType.' - '. $result->cardDetail->cardNumber;
                $payment[0]->card_brand = $result->detailOutput->authorizationCode;
            ...
            if (isset($payment[0])) {
                $payment[0]->transaction_id = $cart->id;
                $payment[0]->card_number = $result->detailOutput->responseCode.' - '. $result->cardDetail->cardNumber;
                $payment[0]->card_brand = $result->detailOutput->authorizationCode;
                $payment[0]->card_expiration = $paymentType;
                $payment[0]->card_holder = '';
                $payment[0]->save();
            }
             */
