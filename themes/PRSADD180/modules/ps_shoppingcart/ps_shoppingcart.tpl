...
<div id="_desktop_cart">
  <div class="blockcart cart-preview {if $cart.products_count > 0}active{else}inactive{/if}" data-refresh-url="{$refresh_url}">
    <div class="header blockcart-header dropdown js-dropdown">
		...
		<a class="carthome" rel="nofollow" href="{$cart_url}" {if $cart.products_count > 0} data-toggle="" aria-haspopup="true" aria-expanded="false"{/if}> Mi Carro
			...
			<div class="block_content">
						...
						{foreach from=$cart.subtotals item="subtotal"}
						  {if $subtotal.value && $subtotal.type !== 'tax'}
							<div class="cart-summary-line" id="cart-subtotal-{$subtotal.type}">
								...
								<span class="label"> {if $subtotal.type != 'shipping'} {$subtotal.label} {else} {if $subtotal.value == 'Gratis' } Retiro Tienda {else} Transporte {/if} {/if}</span>
			  ...
			  <div class="checkout card-block">
				  <a rel="nofollow" href="{$cart_url}" class="viewcart">
				  	<button type="button" class="btn btn-primary viewcart_button">{l s='View Cart' d='Shop.Theme.Actions'}</button>
				  </a>
